...
df = pd.read_csv('data/enriched/SPY.enriched.csv')

#print(df.head())

for i in range(1, len(df)):
    df.loc[i, 'newRSI14'] = df.loc[i-1, 'rsi14']
    df.loc[i, 'newSMA200'] = df.loc[i-1, 'sma200']
    df.loc[i, 'newSMA20'] = df.loc[i-1, 'sma20']
    df.loc[i, 'newMACD'] = df.loc[i-1, 'sma20'] - df.loc[i-1, 'sma100']
    df.loc[i, 'newEMA20'] = df.loc[i-1, 'ema20']
    df.loc[i, 'prevClose'] =  df.loc[i-1, 'close']
    #df.loc[i, 'newRSI'] = df.loc[i-1, 'rsi14']
"""
NUM_PAST_DAYS = 5
for i in range(NUM_PAST_DAYS, len(df)):
    ...
    for j in range(i-NUM_PAST_DAYS, i):
        ...
        ct -= 1
"""
df = df[['close', 'prevClose', 'newRSI14', 'newSMA200', 'newSMA20', 'newMACD', 'newEMA20']]
...
for i in range(0, len(predictions)):
    ...
    yesterdayClose = test_features[i][0]
