...
def emaGradient(path, window):
    print("--------------------")
    print("Running EMA" + str(window) + " Gradient Strategy.")
    print("--------------------")
    emaColumn = "ema" + str(window)
    dataframe = pd.read_csv(path)
    count = len(dataframe)
    ...
    for i in range(1, count):
        if (dataframe.iloc[i-1]["emaColumn"] < dataframe.iloc[i][emaColumn]) and (not long) :
            qty = math.floor(balance/dataframe.iloc[i]['close'])
            print (str(dataframe.iloc[i]["timestamp"]) + ": BUY " + str(qty) + " units at $" + str(dataframe.iloc[i]['close']))
            long = True
            balance -= dataframe.iloc[i]['close'] * qty
            unitsAvailable += qty
            prevEMA = dataframe.iloc[i][emaColumn]
